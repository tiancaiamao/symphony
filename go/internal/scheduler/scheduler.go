...
func (s *Scheduler) handleRunningTask(ctx context.Context, task *store.Task) {
	...
	if !exists {
		// Task is marked running but not in our map - orphaned state
...
func (s *Scheduler) startTask(ctx context.Context, task *store.Task) {
	// Generate workspace
	wsPath := s.workspace.WorkspaceForIssue(task.ID)
...
func (s *Scheduler) runTask(ctx context.Context, rt *RunningTask, task *store.Task, wsPath string) {
	...
	// store.SetTaskPID(s.db, task.ID, pid)
	...
	for event := range rt.Agent.Events() {
		...
		case agent.EventAgentEnd:
			...
			// Transition to Self Review instead of Done
...
func (s *Scheduler) buildPrompt(task *store.Task) string {
	// Try to read WORKFLOW.md from workspace root
	workflowPath := "WORKFLOW.md"
	workflowContent, err := s.workspace.ReadWorkflow(workflowPath)
	if err != nil {
